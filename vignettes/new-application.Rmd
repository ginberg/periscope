...
create_new_application(name = 'mytestapp', location = app_dir, sampleapp = TRUE, reset = FALSE)
...
# application without a reset button
create_new_application(name = 'mytestapp', location = app_dir, reset = FALSE)
# application with a right sidebar using the default icon
...
at the specified location.  The location must exist when calling this function. 
To add functionality from the shinydashbordPlus package set the *dashboard_plus* parameter to TRUE.
...
```{r, eval=F}
# -- Create UI sidebar Elements
s1 <- rightSidebarTabContent(id = 1, icon = "desktop", title = "Tab 1", active = TRUE, 
